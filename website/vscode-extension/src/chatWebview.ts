    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const options = {
                ...
                timeout: 90000 // 90 seconds (local LLMs can be slow to initialize)
            };
...
export class EthrixChatProvider implements vscode.WebviewViewProvider {
    ...
    private async updateExtensionSettings(newSettings: any) {
        const config = vscode.workspace.getConfiguration('ethrix');
    ...
    private async handleChatSubmit(text: string, history: Array<{ role: string, text: string }>) {
        if (!this._view) { return; }
        ...
        // Dynamically collect active workspace and file context to make Ethrix workspace-aware
        ...
        if (workspaceRoot) {
            try {
                ...
                const fileList = files
                    .filter(f => !f.startsWith('.') && f !== 'node_modules' && f !== 'venv' && f !== '.venv' && f !== '__pycache__' && f !== 'dist' && f !== 'out')
                    .slice(0, 35)
                    .map(f => {
                        try {
        ...
        if (workspaceRoot) {
            workspaceContext = `[Workspace Context]\n` +
                ...
                workspaceFiles +
                `- Note: When using filesystem tools like 'read_file', 'write_file', or 'list_directory', you MUST resolve them relative to the active workspace root: ${workspaceRoot} using absolute paths.\n`;
        } else {
            workspaceContext = `[Workspace Context]\n- No workspace folder is open.\n`;
        }

        // Smarter active editor detection: prioritize non-report editor
        const reportFileName = config.get<string>('reportFileName', 'ethrix_report.md');
        let editor = vscode.window.activeTextEditor;
        if (!editor || path.basename(editor.document.fileName) === reportFileName) {
            const nonReportEditor = vscode.window.visibleTextEditors.find(e => 
                path.basename(e.document.fileName) !== reportFileName
            );
            if (nonReportEditor) {
